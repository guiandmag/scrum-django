(function($, Backbone, _, app){
	...
	function csrfSafeMethod(method){
		...
		return (/^(GET|HEAD|OPTIONS|TRACE)$/i.test(methods));
	}
	...
	app.models.Task = BaseModel.extend({
		...
		moveTo: function(status, sprint, order) {
			...
			} else if (updates_status < 2 && this.get('started')) {
	...
	app.collections.ready = $.getJSON(api.apiRoot);
